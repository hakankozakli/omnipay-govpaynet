 ...
 */
/**
 * WsdlClass to communicate with SOAP service
 * 
 * @package 
 * @date 2015-06-04
 */
class WsdlClass extends \stdClass implements \ArrayAccess, \Iterator, \Countable
{
     ...
     * @var string
     */
    const VALUE_WSDL_URL = null;
    /**
     * Option key to define WSDL login
     * @var string
     */
    const WSDL_LOGIN = null;
    /**
    ...
    /**
     * Option key to define WSSE password type
     * @var string
     */
    const WSSE_PASSWORD_TYPE = 'wsse_password_type';
    /**
     * Option key to define WSSE login
     * @var string
     */
    const WSSE_USERNAME = 'wsse_username';
    /**
     * Option key to define WSSE password
     * @var string
     */
    const WSSE_PASSWORD = 'wsse_password';
	/**
	 * WSS specs url
	 * @var  string
	 */
	const WSSE_URL = 'http://docs.oasis-open.org/wss/2004/01';
    /**
    ...
    /**
     * Array with the initial config passed on to the soapClient
     * @var initialConfig
     */
    ...
    public function __construct($_arrayOfValues = array(),$_resetSoapClient = false)
    {
        ...
        if($_resetSoapClient)
        {
			self::$initialConfig = $_arrayOfValues;
            $this->initSoapClient($_arrayOfValues);
		}
		else
		{
			$_arrayOfValues = array_merge(self::$initialConfig, $_arrayOfValues);
		}
    ...
    public static function __set_state(array $_array,$_className = __CLASS__)
    {
        if(class_exists($_className))
        {
            ...
            if(is_object($object) && is_subclass_of($object,'Flexcoders\GovPayNet\\WsdlClass'))
            {
    ...
    {
        if(class_exists('Flexcoders\GovPayNet\\ClassMap',true))
        {
            $wsdlOptions = array();
            $wsdlOptions['classmap'] = ClassMap::classMap();
            $defaultWsdlOptions = self::getDefaultWsdlOptions();
            foreach($defaultWsdlOptions as $optioName=>$optionValue)
            {
				if(strpos($optioName, 'wsdl_') !== 0)
					continue;
            ...
            if(array_key_exists(str_replace('wsdl_','',self::WSDL_URL),$wsdlOptions))
            {
                ...
                $this->generateWsseHeader($_wsdlOptions);
            }
        }
    }
    /**
    ...
    public static function getDefaultWsdlOptions()
    {
        return array(
                    ...
                    self::WSSE_PASSWORD_TYPE=>null,
                    self::WSSE_USERNAME=>null,
                    self::WSSE_PASSWORD=>null,
         );
    }
    /**
	...
	*
     ...
     * @return void
	*/
	protected function generateWsseHeader($_wsdlOptions)
	{
		// do we need to generate a WSSE header?
		if ( ! isset($_wsdlOptions[self::WSSE_PASSWORD_TYPE]) or ! in_array($_wsdlOptions[self::WSSE_PASSWORD_TYPE], array('PasswordText', 'PasswordDigest')))
			return;

		// generate the timestamps
		$timestamp = gmdate('Y-m-d\TH:i:s\Z');
		$expireTimestamp = gmdate('Y-m-d\TH:i:s', time() + 300).'Z';

		// PasswordDigest
		if ($_wsdlOptions[self::WSSE_PASSWORD_TYPE] === 'PasswordDigest')
		{
			$nonce = mt_rand();
			$packedNonce = pack('H*', $nonce);
			$packedTimestamp = pack('a*', $timestamp);
			$packedPassword = pack('a*', $_wsdlOptions[self::WSSE_PASSWORD]);
			$hash = sha1($packedNonce . $packedTimestamp . $packedPassword);
			$password = base64_encode(pack('H*', $hash));
			$nonce = base64_encode(pack('H*',$nonce));
		}

		// PasswordText
		elseif ($_wsdlOptions[self::WSSE_PASSWORD_TYPE] === 'PasswordText')
		{
			$password = $_wsdlOptions[self::WSSE_PASSWORD];
		}

		// construct the XML for the header
		$xml = '
			<wsse:Security SOAP-ENV:mustUnderstand="1" xmlns:wsse="'.self::WSSE_URL.'/oasis-200401-wss-wssecurity-secext-1.0.xsd" xmlns:wsu="'.self::WSSE_URL.'/oasis-200401-wss-wssecurity-utility-1.0.xsd">';

		// for digest authentication, add the timestamp to the XML
		if ($_wsdlOptions[self::WSSE_PASSWORD_TYPE] === 'PasswordDigest')
		{
			$xml .= '
		...
		$xml .= '
				<wsse:UsernameToken xmlns:wsu="'.self::WSSE_URL.'/oasis-200401-wss-wssecurity-utility-1.0.xsd">
					<wsse:Username>'.$_wsdlOptions[self::WSSE_USERNAME].'</wsse:Username>
					<wsse:Password Type="'.self::WSSE_URL.'/oasis-200401-wss-username-token-profile-1.0#'.$_wsdlOptions[self::WSSE_PASSWORD_TYPE].'">'.$password.'</wsse:Password>';

		// for digest authentication, add the nonce to the XML
		if ($_wsdlOptions[self::WSSE_PASSWORD_TYPE] === 'PasswordDigest')
		{
			$xml .= '
					<wsse:Nonce EncodingType="'.self::WSSE_URL.'/oasis-200401-wss-soap-message-security-1.0#Base64Binary">'.$nonce.'</wsse:Nonce>
					<wsu:Created xmlns:wsu="'.self::WSSE_URL.'/oasis-200401-wss-wssecurity-utility-1.0.xsd">'.$timestamp.'</wsu:Created>';
		}
		...
		// construct the soap header object
		$this->setSoapHeader(
			self::WSSE_URL.'/oasis-200401-wss-wssecurity-secext-1.0.xsd',
			'Security',
